...
class ControllerPaymentEComProcessingCheckout extends Controller
{
	 ...
	 * @var string
	 */
	private $module_name = 'ecomprocessing_checkout';
	...
	public function index()
	{
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$data = array(
				...
				'transaction_types' => $this->model_payment_ecomprocessing_checkout->getTransactionTypes(),
				...
				'action' => $this->url->link('payment/ecomprocessing_checkout', 'token=' . $this->session->data['token'], 'SSL'),
				...
				// Settings
				'ecomprocessing_checkout_username' => $this->getFieldValue('ecomprocessing_checkout_username'),
				'ecomprocessing_checkout_password' => $this->getFieldValue('ecomprocessing_checkout_password'),
				'ecomprocessing_checkout_sandbox' => $this->getFieldValue('ecomprocessing_checkout_sandbox'),
				'ecomprocessing_checkout_transaction_type' => $this->getFieldValue('ecomprocessing_checkout_transaction_type'),
				'ecomprocessing_checkout_total' => $this->getFieldValue('ecomprocessing_checkout_total'),
				'ecomprocessing_checkout_order_status_id' => $this->getFieldValue('ecomprocessing_checkout_order_status_id'),
				'ecomprocessing_checkout_order_failure_status_id' => $this->getFieldValue('ecomprocessing_checkout_order_failure_status_id'),
				'ecomprocessing_checkout_geo_zone_id' => $this->getFieldValue('ecomprocessing_checkout_geo_zone_id'),
				'ecomprocessing_checkout_status' => $this->getFieldValue('ecomprocessing_checkout_status'),
				'ecomprocessing_checkout_sort_order' => $this->getFieldValue('ecomprocessing_checkout_sort_order'),
				'ecomprocessing_checkout_debug' => $this->getFieldValue('ecomprocessing_checkout_debug'),
				'ecomprocessing_checkout_supports_partial_capture' => $this->getFieldValue('ecomprocessing_checkout_supports_partial_capture'),
				'ecomprocessing_checkout_supports_partial_refund' => $this->getFieldValue('ecomprocessing_checkout_supports_partial_refund'),
				'ecomprocessing_checkout_supports_void' => $this->getFieldValue('ecomprocessing_checkout_supports_void'),
			);

			$defaultParamValues = array(
				'ecomprocessing_checkout_sandbox'                  => 1,
				'ecomprocessing_checkout_status'                   => 0,
				'ecomprocessing_checkout_debug'                    => 1,
				'ecomprocessing_checkout_supports_partial_capture' => 1,
				'ecomprocessing_checkout_supports_partial_refund'  => 1,
				'ecomprocessing_checkout_supports_void'            => 1
			);
			...
			$data['breadcrumbs'][] = array(
				'text' => $headingTitle,
				'href' => $this->url->link('payment/ecomprocessing_checkout', 'token=' . $this->session->data['token'], 'SSL')
			);

			$this->response->setOutput(
				$this->load->view('payment/ecomprocessing_checkout.tpl', $data)
			);
		}
	}
	...
	{
		if ($this->config->get('ecomprocessing_checkout_status')) {
			...
			$transactions = $this->model_payment_ecomprocessing_checkout->getTransactionsByOrder($orderId);
			...
			if ($transactions) {
				...
				foreach ($transactions as &$transaction) {
					...
					if (in_array($transaction['type'], array('authorize', 'authorize3d', 'capture', 'sale', 'sale3d', 'init_recurring_sale', 'recurring_sale', 'refund')) && $transaction) {
						...
						$transaction['void_exists'] = $this->model_payment_ecomprocessing_checkout->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved') !== false;
					} else {
				...
				$data = array(
					...
					'ecomprocessing_checkout_supports_void'            => $this->config->get('ecomprocessing_checkout_supports_void'),

					'order_id'                                  => $orderId,
					'token'                                     => $this->request->get['token'],
					'url_modal'                                 => htmlspecialchars_decode($this->url->link('payment/ecomprocessing_checkout/getModalForm', 'token=' . $this->session->data['token'], 'SSL')),
					'module_name'				                => $this->module_name,
					'currency'		                            => $this->getTemplateCurrencyArray(),
					'transactions'                              => $transactions,
				);

				return $this->load->view('payment/ecomprocessing_checkout_order.tpl', $data);
			}
		}
	...
	public function getModalForm()
	{
		if (isset($this->request->post['reference_id']) && isset($this->request->post['type'])) {
			...
			$transaction = $this->model_payment_ecomprocessing_checkout->getTransactionById($ref_id);

			if ($type == 'capture') {
				$totalAuthorizedAmount           = $this->model_payment_ecomprocessing_checkout->getTransactionsSumAmount($orderId, $transaction['reference_id'], array('authorize', 'authorize3d'), 'approved');
				$totalCapturedAmount             = $this->model_payment_ecomprocessing_checkout->getTransactionsSumAmount($orderId, $transaction['unique_id'], 'capture', 'approved');
				$transaction['available_amount'] = $totalAuthorizedAmount - $totalCapturedAmount;
			}
			else if ($type == 'refund') {
				$hasVoidTransaction = $this->model_payment_ecomprocessing_checkout->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved');
				if (!$hasVoidTransaction) {
					$totalCapturedAmount             = $transaction['amount'];
					$totalRefundedAmount             = $this->model_payment_ecomprocessing_checkout->getTransactionsSumAmount($orderId, $transaction['unique_id'], 'refund', 'approved');
				...
				$transaction['is_allowed'] = $this->model_payment_ecomprocessing_checkout->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved') == false;
			}

			$url_action = $this->url->link('payment/ecomprocessing_checkout/' . $type, 'token=' . $this->session->data['token'], 'SSL');
			...
			$data = array(
				...
				'ecomprocessing_checkout_supports_partial_capture'   => $this->config->get('ecomprocessing_checkout_supports_partial_capture'),
				'ecomprocessing_checkout_supports_partial_refund'    => $this->config->get('ecomprocessing_checkout_supports_partial_refund'),
				'ecomprocessing_checkout_supports_void'              => $this->config->get('ecomprocessing_checkout_supports_void')

			);

			$this->response->setOutput(
				$this->load->view('payment/ecomprocessing_checkout_order_modal.tpl', $data)
			);
		}
	}
	...
	public function capture()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			$transaction = $this->model_payment_ecomprocessing_checkout->getTransactionById($this->request->post['reference_id']);
			...
			if (isset($transaction['order_id']) && abs((int)$transaction['order_id']) > 0) {
				...
				$capture = $this->model_payment_ecomprocessing_checkout->capture($transaction['unique_id'], $amount, $transaction['currency'], $message, $transaction['terminal_token']);
	...
	public function refund()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && abs((int)$transaction['order_id']) > 0) {
				...
				$refund = $this->model_payment_ecomprocessing_checkout->refund($transaction['unique_id'], $amount, $transaction['currency'], $message, $transaction['terminal_token']);
	...
	public function void()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && abs((int)$transaction['order_id']) > 0) {
				...
				$void = $this->model_payment_ecomprocessing_checkout->void($transaction['unique_id'], $message, $transaction['terminal_token']);
	...
	{
		$this->load->model('payment/ecomprocessing_checkout');
		$this->model_payment_ecomprocessing_checkout->install();
	}
	...
	{
		$this->load->model('payment/ecomprocessing_checkout');
		$this->model_payment_ecomprocessing_checkout->uninstall();
	}
	...
	{
		if (!$this->user->hasPermission('modify', 'payment/ecomprocessing_checkout')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (@empty($this->request->post['ecomprocessing_checkout_username'])) {
			$this->error['username'] = $this->language->get('error_username');
		}

		if (@empty($this->request->post['ecomprocessing_checkout_password'])) {
			$this->error['password'] = $this->language->get('error_password');
		}

		if (@empty($this->request->post['ecomprocessing_checkout_transaction_type'])) {
	...
	protected function isUserLoggedInAndAuthorized()
	{
		...
		$hasAccess  = $this->user->hasPermission('access', 'payment/ecomprocessing_checkout');
