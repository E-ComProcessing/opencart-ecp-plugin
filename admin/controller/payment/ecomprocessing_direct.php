...
class ControllerPaymentEComProcessingDirect extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('ecomprocessing_direct', $this->request->post);
		...
		$this->response->setOutput($this->load->view('payment/ecomprocessing_direct.tpl', $data));
	}
		...
		if ($this->config->get('ecomprocessing_direct_status')) {
			...
			if ($transactions) {
				...
				$data = array(
					...
					'url_modal'     => htmlspecialchars_decode($this->url->link('payment/ecomprocessing_direct/getModalForm', 'token=' . $this->session->data['token'], 'SSL')),
	...
	public function getModalForm() {
		if (isset($this->request->post['reference_id']) && isset($this->request->post['type'])) {
			...
			$data = array(
				...
				'url_action' => $this->url->link('payment/ecomprocessing_direct/' . $type, 'token=' . $this->session->data['token'], 'SSL'),
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (is_array($transaction)) {
				...
				if (isset($capture->response) && !$capture->error) {
					...
					$amount = $this->model_payment_ecomprocessing_direct->iso4217ConvertAmount($capture->response->amount, $capture->response->currency);
					...
					$this->model_payment_ecomprocessing_direct->addTransaction($data);
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				if (isset($refund->response) && !$refund->error) {
					$amount = $this->model_payment_ecomprocessing_direct->iso4217ConvertAmount($refund->response->amount, $refund->response->currency);
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				if (isset($void->response) && !$void->error) {
					$amount = $this->model_payment_ecomprocessing_direct->iso4217ConvertAmount($void->response->amount, $void->response->currency);
