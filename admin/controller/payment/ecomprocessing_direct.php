 ...
 */
class ControllerPaymentEComProcessingDirect extends Controller
{
	/**
	 * Error storage
	 *
	 ...
	 * @var string
	 */
	private $module_name = 'ecomprocessing_direct';
	...
	public function index()
	{
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$this->load->model('payment/ecomprocessing_direct');
			...
			$data = array(
				...
				'transaction_types' => $this->model_payment_ecomprocessing_direct->getTransactionTypes(),
				...
				// Settings
				'ecomprocessing_direct_username'                 => $this->getFieldValue('ecomprocessing_direct_username'),
				'ecomprocessing_direct_password'                 => $this->getFieldValue('ecomprocessing_direct_password'),
				'ecomprocessing_direct_token'                    => $this->getFieldValue('ecomprocessing_direct_token'),
				'ecomprocessing_direct_sandbox'                  => $this->getFieldValue('ecomprocessing_direct_sandbox'),
				'ecomprocessing_direct_transaction_type'         => $this->getFieldValue('ecomprocessing_direct_transaction_type'),
				'ecomprocessing_direct_total'                    => $this->getFieldValue('ecomprocessing_direct_total'),
				'ecomprocessing_direct_order_status_id'          => $this->getFieldValue('ecomprocessing_direct_order_status_id'),
				'ecomprocessing_direct_failure_order_status_id'  => $this->getFieldValue('ecomprocessing_direct_failure_order_status_id'),
				'ecomprocessing_direct_async_order_status_id'    => $this->getFieldValue('ecomprocessing_direct_async_order_status_id'),
				'ecomprocessing_direct_geo_zone_id'              => $this->getFieldValue('ecomprocessing_direct_geo_zone_id'),
				'ecomprocessing_direct_status'                   => $this->getFieldValue('ecomprocessing_direct_status'),
				'ecomprocessing_direct_sort_order'               => $this->getFieldValue('ecomprocessing_direct_sort_order'),
				'ecomprocessing_direct_debug'                    => $this->getFieldValue('ecomprocessing_direct_debug'),
				'ecomprocessing_direct_supports_partial_capture' => $this->getFieldValue('ecomprocessing_direct_supports_partial_capture'),
				'ecomprocessing_direct_supports_partial_refund'  => $this->getFieldValue('ecomprocessing_direct_supports_partial_refund'),
				'ecomprocessing_direct_supports_void'            => $this->getFieldValue('ecomprocessing_direct_supports_void'),

				'action' => $this->url->link('payment/ecomprocessing_direct', 'token=' . $this->session->data['token'], 'SSL'),
			...
			$defaultParamValues = array(
				'ecomprocessing_direct_sandbox'                  => 1,
				'ecomprocessing_direct_status'                   => 0,
				'ecomprocessing_direct_debug'                    => 1,
				'ecomprocessing_direct_supports_partial_capture' => 1,
				'ecomprocessing_direct_supports_partial_refund'  => 1,
				'ecomprocessing_direct_supports_void'            => 1
			);
			...
			$data['breadcrumbs'][] = array(
				'text' => $headingTitle,
				'href' => $this->url->link('payment/ecomprocessing_direct', 'token=' . $this->session->data['token'], 'SSL')
			);

			$this->response->setOutput(
				$this->load->view('payment/ecomprocessing_direct.tpl', $data)
			);
		}
	}
	...
	{
		if ($this->config->get('ecomprocessing_direct_status')) {
			...
			$transactions = $this->model_payment_ecomprocessing_direct->getTransactionsByOrder($this->request->get['order_id']);
			...
			if ($transactions) {
				...
				foreach ($transactions as &$transaction) {
					...
					if (in_array($transaction['type'], array('authorize', 'authorize3d', 'capture', 'sale', 'sale3d', 'init_recurring_sale', 'recurring_sale', 'refund')) && $transaction) {
						...
						$transaction['void_exists'] = $this->model_payment_ecomprocessing_direct->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved') !== false;
					} else {
				...
				$data = array(
					...
					'ecomprocessing_direct_supports_void'              => $this->config->get('ecomprocessing_direct_supports_void'),

					'order_id'                   => $orderId,
					'token'                      => $this->request->get['token'],
					'url_modal'                  => htmlspecialchars_decode(
						$this->url->link(
							'payment/ecomprocessing_direct/getModalForm', 'token=' . $this->session->data['token'], 'SSL'
						)
					),
					'module_name'                => $this->module_name,
				...
				return $this->load->view('payment/ecomprocessing_direct_order.tpl', $data);
			}
		}
	...
	public function getModalForm()
	{
		if (isset($this->request->post['reference_id']) && isset($this->request->post['type'])) {
			...
			$transaction = $this->model_payment_ecomprocessing_direct->getTransactionById($reference_id);

			if ($type == 'capture') {
				$totalAuthorizedAmount           = $this->model_payment_ecomprocessing_direct->getTransactionsSumAmount($orderId, $transaction['reference_id'], array('authorize', 'authorize3d'), 'approved');
				$totalCapturedAmount             = $this->model_payment_ecomprocessing_direct->getTransactionsSumAmount($orderId, $transaction['unique_id'], 'capture', 'approved');
				$transaction['available_amount'] = $totalAuthorizedAmount - $totalCapturedAmount;
			}
			else if ($type == 'refund') {
				$hasVoidTransaction = $this->model_payment_ecomprocessing_direct->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved');
				if (!$hasVoidTransaction) {
					$totalCapturedAmount             = $transaction['amount'];
					$totalRefundedAmount             = $this->model_payment_ecomprocessing_direct->getTransactionsSumAmount($orderId, $transaction['unique_id'], 'refund', 'approved');
				...
				$transaction['is_allowed'] = $this->model_payment_ecomprocessing_direct->getTransactionsByTypeAndStatus($orderId, $transaction['unique_id'], 'void', 'approved') == false;
			}

			$url_action = $this->url->link(
				'payment/ecomprocessing_direct/' . $type, 'token=' . $this->session->data['token'], 'SSL'
			);
			...
			$data = array(
				...
				'ecomprocessing_direct_supports_partial_capture'   => $this->config->get('ecomprocessing_direct_supports_partial_capture'),
				'ecomprocessing_direct_supports_partial_refund'    => $this->config->get('ecomprocessing_direct_supports_partial_refund'),
				'ecomprocessing_direct_supports_void'              => $this->config->get('ecomprocessing_direct_supports_void')

			);

			$this->response->setOutput($this->load->view('payment/ecomprocessing_direct_order_modal.tpl', $data));
		}
	}
	...
	public function capture()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			$transaction = $this->model_payment_ecomprocessing_direct->getTransactionById($this->request->post['reference_id']);
			...
			if (isset($transaction['order_id']) && abs((int)$transaction['order_id']) > 0) {
				...
				$capture = $this->model_payment_ecomprocessing_direct->capture($transaction['unique_id'], $amount, $transaction['currency'], $message);
				...
				if (isset($capture->unique_id)) {
					...
					$this->model_payment_ecomprocessing_direct->populateTransaction($data);
	...
	public function refund()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				$refund = $this->model_payment_ecomprocessing_direct->refund($transaction['unique_id'], $amount, $transaction['currency'], $message);
	...
	public function void()
	{
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && abs((int)$transaction['order_id']) > 0) {
				...
				$void = $this->model_payment_ecomprocessing_direct->void($transaction['unique_id'], $message);
	...
	{
		$this->load->model('payment/ecomprocessing_direct');
		$this->model_payment_ecomprocessing_direct->install();
	}
	...
	{
		$this->load->model('payment/ecomprocessing_direct');
		$this->model_payment_ecomprocessing_direct->uninstall();
	}
	...
	{
		if (!$this->user->hasPermission('modify', 'payment/ecomprocessing_direct')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (@empty($this->request->post['ecomprocessing_direct_username'])) {
			$this->error['username'] = $this->language->get('error_username');
		}

		if (@empty($this->request->post['ecomprocessing_direct_password'])) {
			$this->error['password'] = $this->language->get('error_password');
		}

		if (@empty($this->request->post['ecomprocessing_direct_token'])) {
			$this->error['token'] = $this->language->get('error_token');
		}

		if (@empty($this->request->post['ecomprocessing_direct_transaction_type'])) {
	...
	protected function isUserLoggedInAndAuthorized()
	{
		...
		$hasAccess  = $this->user->hasPermission('access', 'payment/ecomprocessing_direct');
