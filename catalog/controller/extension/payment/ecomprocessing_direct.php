...
class ControllerExtensionPaymentEComprocessingDirect extends ControllerExtensionPaymentEComprocessingBase
{
	...
	public function index()
	{
		...
		if ($this->model_extension_payment_ecomprocessing_direct->isCartContentMixed()) {
			$template = 'ecomprocessing_disabled.tpl';
			$data = $this->prepareViewDataMixedCart();

		} else {
			$template = 'ecomprocessing_direct.tpl';
			$this->document->addScript(
				'catalog/view/javascript/ecomprocessing/card.min.js'
			);
	...
	public function prepareViewData()
	{
		$data = array(
			...
			'button_target' => $this->url->link('extension/payment/ecomprocessing_direct/send', '', 'SSL'),
	...
	public function send()
	{
		...
		try {
			...
			$data = array(
				'transaction_id' => $this->model_extension_payment_ecomprocessing_direct->genTransactionId(self::PLATFORM_TRANSACTION_PREFIX),

				'remote_address' => $this->request->server['REMOTE_ADDR'],

				'usage' => $this->model_extension_payment_ecomprocessing_direct->getUsage(),
				'description' => $this->model_extension_payment_ecomprocessing_direct->getOrderProducts(
					$this->session->data['order_id']
				),

				'currency' => $this->model_extension_payment_ecomprocessing_direct->getCurrencyCode(),
				...
				'notification_url' => $this->url->link('extension/payment/ecomprocessing_direct/callback', '', 'SSL'),
				'return_success_url' => $this->url->link('extension/payment/ecomprocessing_direct/success', '', 'SSL'),
				'return_failure_url' => $this->url->link('extension/payment/ecomprocessing_direct/failure', '', 'SSL'),
			...
			$transaction = $this->model_extension_payment_ecomprocessing_direct->sendTransaction($data);
			...
			if (isset($transaction->unique_id)) {
				...
				$this->model_extension_payment_ecomprocessing_direct->populateTransaction($data);
				...
				if ($this->model_extension_payment_ecomprocessing_direct->isRecurringOrder()) {
					$this->addOrderRecurring($transaction->unique_id);
					$this->model_extension_payment_ecomprocessing_direct->populateRecurringTransaction($data);
					$this->model_extension_payment_ecomprocessing_direct->updateOrderRecurring($data);
				}
			...
			$this->model_extension_payment_ecomprocessing_direct->logEx($exception);
		}
	...
	public function callback()
	{
		...
		try {
			$this->model_extension_payment_ecomprocessing_direct->bootstrap();
			...
			if ($notification->isAuthentic()) {
				...
				if (isset($reconcile->unique_id)) {
					...
					$transaction = $this->model_extension_payment_ecomprocessing_direct->getTransactionById($reconcile->unique_id);
	...
	public function addOrderRecurring($payment_reference)
	{
		...
		if (!empty($recurring_products)) {
			$this->load->model('extension/payment/ecomprocessing_direct');
			$this->model_extension_payment_ecomprocessing_direct->addOrderRecurring(
				$recurring_products,
				$payment_reference
			);
		}
	...
	{
		$this->load->model('extension/payment/ecomprocessing_direct');
		$this->model_extension_payment_ecomprocessing_direct->processRecurringOrders();
	}
}
