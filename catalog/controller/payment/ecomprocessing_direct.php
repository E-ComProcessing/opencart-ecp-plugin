...
class ControllerPaymentEComProcessingDirect extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ecomprocessing_direct.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/ecomprocessing_direct.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/ecomprocessing_direct.tpl', $data);
		}
	}
	...
	public function send() {
		...
		try {
			...
			$data = array(
				...
				'card_holder'       => $this->request->post['ecomprocessing_direct-cc-holder'],
				'card_number'       => $this->ccFilter($this->request->post['ecomprocessing_direct-cc-number'], 'number'),
				'cvv'               => $this->ccFilter($this->request->post['ecomprocessing_direct-cc-cvv'], 'cvv'),
				'expiration_month'  => $this->ccFilter($this->request->post['ecomprocessing_direct-cc-expiration'], 'month'),
				'expiration_year'   => $this->ccFilter($this->request->post['ecomprocessing_direct-cc-expiration'], 'year'),
			...
			$transaction = $this->model_payment_ecomprocessing_direct->sendTransaction($data);

			if (isset($transaction->response)) {
				$amount = $this->model_payment_ecomprocessing_direct->convertCurrency(
	...
	public function callback() {
		...
		try {
			$this->model_payment_ecomprocessing_direct->bootstrap();
			...
			if ( $notification->isAuthentic() || !$notification->isAuthentic() ) {
				$reconcile = $this->model_payment_ecomprocessing_direct->reconcile(
					$notification->getParsedNotification()->unique_id
				);

				if (isset($reconcile->response)) {

					$transaction = $this->model_payment_ecomprocessing_direct->getTransactionById($reconcile->response->unique_id);
					...
					if ( isset( $transaction['order_id'] ) && intval($transaction['order_id']) > 0 ) {
						...
						$transactionEntry = $this->model_payment_ecomprocessing_direct->getTransactionById(
						...
						if (isset($transactionEntry['order_id'])) {
							...
							$this->model_payment_ecomprocessing_direct->editTransaction($data);
							...
							if ($reconcile->response->status == 'approved') {
								...
								$this->model_checkout_order->addOrderHistory(
									$transaction['order_id'],
									$this->config->get('ecomprocessing_direct_failure_order_status_id'),
			...
			$this->model_payment_ecomprocessing_direct->logEx($exception);
		}
	}
	...
	public function callback() {
		...
		try {
			if (!class_exists('\Genesis\Genesis')) {
				$this->model_payment_ecomprocessing_direct->bootstrapGenesis();
			}
			...
			if ( $notification->isAuthentic() ) {
				$reconcile = $this->model_payment_ecomprocessing_direct->reconcile($notification->getParsedNotification()->unique_id);

				$transaction = $this->model_payment_ecomprocessing_direct->getTransactionById($reconcile->unique_id);

				if ( isset( $transaction['order_id'] ) ) {

					if (strval($reconcile->status) == 'approved') {
						$status_id = $this->config->get( 'ecomprocessing_direct_order_status_id' );
					...
					$data = array(
						...
						'amount'            => $this->model_payment_ecomprocessing_direct->convertCurrency($reconcile->amount, $reconcile->currency),
					...
					$this->model_payment_ecomprocessing_direct->updateTransaction($data);
